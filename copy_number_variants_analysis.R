...
for(chr in chr_list){
  ...
  dir.create(file.path(out_dir, "averaged_threshold5"), showWarnings=FALSE,recursive=TRUE)

  #chr_plot <- file.path(out_dir, "threshold5", paste0("Coverage_", chr, "_threshold5.pdf"))
  chr_plot <- file.path(out_dir, "averaged_threshold5", paste0("Coverage_", chr, "_averaged_threshold5.pdf"))
